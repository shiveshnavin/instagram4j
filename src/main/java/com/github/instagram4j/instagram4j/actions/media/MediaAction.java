...
public class MediaAction {
    ...
    public CompletableFuture<MediaGetCommentsResponse> commentsPage() {
        return new MediaGetCommentsRequest(media_id).execute(client);
    }

    public CompletableFuture<MediaGetCommentsResponse> nextCommentsPage(MediaGetCommentsResponse previous) {
        MediaGetCommentsRequest request = new MediaGetCommentsRequest(media_id);
        request.setMin_id(previous.getNext_min_id());
        request.setSortOrder(SortOrder.valueOfString(previous.getSort_order()));
        return request.execute(client);
    }

    public CompletableFuture<MediaGetCommentsResponse> commentsPage(SortOrder sortOrder){
        MediaGetCommentsRequest request = new MediaGetCommentsRequest(media_id);
